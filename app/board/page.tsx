...
const SPIRITS: Record<string,{name:string;emoji:string;desc:string}> = {
  elder:{name:"The Elder",emoji:"👁️",desc:"Ancient. Cryptic. All-knowing."},
  shadow:{name:"The Shadow",emoji:"🌑",desc:"Dark truths. Ominous answers."},
  oracle:{name:"The Oracle",emoji:"🔮",desc:"Prophetic riddles. Future sight."},
  jester:{name:"The Jester",emoji:"🃏",desc:"Dark humor. Twisted wisdom."},
};
...
function getPos(char:string):{x:number;y:number}|null{
  ...
  if(char==="Y")return{x:22,y:10};if(char==="N")return{x:78,y:10};
  return null;
}
    ...
    <div style={{position:"relative",width:"100%",maxWidth:"680px",margin:"0 auto",background:"linear-gradient(145deg,#2a1f0e,#1a1208,#221a0c)",border:"2px solid rgba(201,168,92,0.35)",borderRadius:"10px",padding:"clamp(1rem,3vw,1.75rem)",boxShadow:"0 0 60px rgba(201,168,92,0.12),inset 0 0 40px rgba(0,0,0,0.5)"}}>
      ...
      {ROWS.map((row,ri)=>(
        <div key={ri} style={{display:"flex",justifyContent:"center",gap:"clamp(0.15rem,1.2vw,0.75rem)",marginBottom:"0.25rem"}}>
          {row.map(l=><span key={l} style={{fontFamily:"var(--font-display)",fontSize:"clamp(0.85rem,2.5vw,1.35rem)",color:lit.has(l)?"#fff":"rgba(201,168,92,0.6)",textShadow:lit.has(l)?"0 0 15px #c9a85c,0 0 30px rgba(201,168,92,0.4)":"none",transition:"all 0.2s",minWidth:"1.1em",textAlign:"center"}}>{l}</span>)}
        </div>
      ))}
      ...
      <div style={{position:"absolute",left:`${pos.x}%`,top:`${pos.y}%`,transform:"translate(-50%,-50%)",width:"clamp(36px,7vw,56px)",height:"clamp(36px,7vw,56px)",background:"radial-gradient(circle,rgba(201,168,92,0.25) 0%,rgba(201,168,92,0.04) 70%)",border:"2px solid rgba(201,168,92,0.55)",borderRadius:"50% 50% 50% 0",transition:"left 0.65s cubic-bezier(0.25,0.46,0.45,0.94),top 0.65s cubic-bezier(0.25,0.46,0.45,0.94)",boxShadow:"0 0 18px rgba(201,168,92,0.35)",pointerEvents:"none"}}>
        <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:"7px",height:"7px",borderRadius:"50%",border:"2px solid rgba(201,168,92,0.75)",background:"transparent"}}/>
      </div>
    </div>
  );
}
export default function BoardPage(){
  ...
  async function ask(){
    ...
    }catch{setAnswer("The spirits are silent... try again.");}
    setLoading(false);
  }
  return(
    <main style={{minHeight:"100dvh",background:"var(--bg)",padding:"0 1rem 5rem"}}>
      ...
      <div style={{maxWidth:"720px",margin:"2rem auto 0"}}>
        <p style={{fontFamily:"var(--font-display)",textAlign:"center",color:"var(--text-muted)",fontSize:"0.65rem",letterSpacing:"0.3em",marginBottom:"1.75rem"}}>CHOOSE YOUR SPIRIT · {left} FREE SESSIONS REMAINING TODAY</p>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"0.65rem",marginBottom:"1.75rem"}}>
          {Object.entries(SPIRITS).map(([k,s])=>(
            <button key={k} onClick={()=>setSpirit(k)} style={{padding:"0.7rem 0.4rem",background:spirit===k?"rgba(201,168,92,0.12)":"var(--surface)",border:`1px solid ${spirit===k?"rgba(201,168,92,0.45)":"rgba(201,168,92,0.1)"}`,borderRadius:"4px",cursor:"pointer",textAlign:"center",transition:"all 0.2s"}}>
        ...
        <div style={{marginTop:"1.75rem",display:"flex",flexDirection:"column",gap:"0.65rem"}}>
          ...
          <button onClick={ask} disabled={loading||!q.trim()} style={{padding:"1rem",background:loading?"rgba(201,168,92,0.25)":"var(--gold)",color:"#070608",fontFamily:"var(--font-display)",fontSize:"0.8rem",letterSpacing:"0.2em",border:"none",borderRadius:"2px",fontWeight:700,transition:"all 0.2s"}}>{loading?"THE SPIRITS STIR...":"ASK THE SPIRITS"}</button>
        </div>
        <p style={{textAlign:"center",marginTop:"1.5rem",fontSize:"0.65rem",color:"var(--text-muted)",letterSpacing:"0.08em",opacity:0.45}}>For entertainment purposes only.</p>
      </div>
      {wall&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:"2rem"}}>
        <div style={{background:"var(--surface-2)",border:"1px solid rgba(201,168,92,0.28)",borderRadius:"8px",padding:"3rem 2rem",maxWidth:"380px",textAlign:"center"}}>
          <p style={{fontFamily:"var(--font-display)",color:"var(--gold)",letterSpacing:"0.2em",fontSize:"0.65rem",marginBottom:"1rem"}}>THE VEIL HAS CLOSED</p>
          <h2 style={{fontFamily:"var(--font-display)",color:"var(--gold)",fontSize:"1.6rem",marginBottom:"1rem"}}>Free sessions exhausted</h2>
          <p style={{color:"var(--text-muted)",lineHeight:1.7,marginBottom:"2rem"}}>The spirits demand tribute. Upgrade to continue.</p>
